...
def main(config):
    ...
    # select the related labels###############################################
    select_label = random.sample(range(0, 64), 20)    
    fix_label = np.arange(20)
    
    # index of train data with original selected labels
    train_idx = []
    for A in select_label:
    ...
    train_dataset.label = train_dataset.label[train_idx]
    
    # update the label to 0,1,...,19
    for i in range(len(select_label)):
        train_dataset.label[train_dataset.label == select_label[i]] = - fix_label[i]
    train_dataset.label = - train_dataset.label
    train_dataset.label = train_dataset.label.tolist()
        
    utils.log('train dataset: {} (x{})'.format(
            train_dataset[0][0].shape, len(train_dataset.label)))
...
if __name__ == '__main__':
    ...
    config = yaml.load(open('configs/train_meta_mini.yaml'), Loader=yaml.FullLoader)
